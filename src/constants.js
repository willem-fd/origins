...
// Flags are derived from the code (each letter -> Regional Indicator Symbol).
// The list is alphabetized at module load.
const codeToFlag = code => code.toUpperCase().split('').map(c => String.fromCodePoint(0x1F1E6 + c.charCodeAt(0) - 65)).join('')
...
export const validatePhone = p => /^[+\d\s\-().]{6,}$/.test(p || '')
